...
class ReservationBalanceController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        return DataTables::of($query)
            ->addColumn('total_sum', function (Reservation $reservation) {
                return collect($reservation->purchases)
                    ->map(fn(Purchase $purchase) => $purchase->quantity * $purchase->product->price)
                    ->sum();
            })
            ...
            ->addColumn('actions', function (Reservation $reservation) {
                return '<div>
                            <a href="' .
                    route('reservation-balance.show', $reservation) .
                    '" class="btn btn-secondary btn-sm">View</a>
                            <button type="button" onclick="document.getElementById(' .
                    "'deleteForm'" .
                    ').submit()" class="btn btn-dark btn-sm">Delete</button>
                            <form action="' .
                    route('reservation-balance.destroy', $reservation) .
    ...
    public function update(UpdateReservationBalanceRequest $request, Reservation $reservationBalance): RedirectResponse
    {
        if ($request->amount > $reservationBalance?->remaining_balance) {
            return redirect()
                ->back()
                ->withErrors(['message' => 'Cannot deposit larger than remaining balance.']);
        }

        $remainingBalanceAmount = $reservationBalance->remaining_balance - $request->amount;
        $reservationBalance->update(['remaining_balance' => $remainingBalanceAmount]);
        
        $reservationBalance->payments()->create([
